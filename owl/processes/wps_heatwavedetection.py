...
class HWs_detection(Process):
    ...
    def __init__(self):
        ...
        outputs = [
            ...
            ComplexOutput(
                "output",
                "netCDF containing a Heatwave index",
            ...
            ComplexOutput(
                "plot",
                "Preview: Graphical visualisation of the Heatwave",
        ...
        super(HWs_detection, self).__init__(
            ...
            title="HWs_detection",
            version="0.1.0",
            # more explaionation is useful here
    ...
    def _handler(self, request, response):
        ...
        try:
            ...
            data = np.zeros(
                (nlon, nlat, nyear, nday, 1)
            )  # daily data for all the years
        ...
        try:
            for iyear, year in enumerate(range(ref_year1, ref_year2 + 1)):
                ...
                # selecting BBox:
                obs1 = obs1.sel(
                    latitude=slice(lats_bnds[1], lats_bnds[0]),
                    longitude=slice(lons_bnds[0], lons_bnds[1]),
                )
                # selecting time and bbox
                obs = obs1.sel(time=obs1.time.time.dt.year.isin(year))
                obs = obs.sel(
                    time=~((obs.time.dt.month == 2) & (obs.time.dt.day == 29))
                )
                data[:, :, iyear, :, 0] = np.transpose(
                    obs.to_array()[0, :, :, :], [2, 1, 0]
                )
        ...
        def parallelized_HWMIs_computation(
            ilat, ilon
        ):  # , HWMI, ndayexedthreshold, DDthreshold, fitsubHWarray, subHWarray):
            (
                HWMIyear,
                HWlstyear,
                HWstartmembyear,
                HWendmembyear,
                ndayexedthresholdyear,
                DDthresholdyear,
                subHWarrayyear,
                fitsubHWarrayyear,
                sstMeanarrayyear,
        ...
        try:
            ...
            # Parallel(n_jobs=-1, timeout = 5*3600, verbose = 20, require='sharedmem', mmap_mode='w+')(delayed(parallelized_HWMIs_computation)(ilat, ilon) for ilat in range(nlat) for ilon in range(nlon))  # noqa
        ...
        for i, iyear in enumerate(range(ref_year1, ref_year2 + 1)):
            ...
            try:
                parameters_str = (
                    ...
                    + "_"
                    + cv_str
                    + "_percent%i" % (percent_thresh)
                    + "_daymin%i" % (duration_min)
                    + "_ref_"
                    + str(ref_year1)
                    + "_"
                    + str(ref_year2)
                ...
                fileout = tempfile.mktemp(
                    suffix=".nc", prefix="heatwaveindex_", dir=workdir
                )
            ...
            try:
                ...
                times[:] = date2num(
                    datetime(iyear, season_start_day[0], season_start_day[1]),
                    units=times.units,
            ...
            try:
                ...
                HWMIfile = fout.createVariable(
                    vout1, np.float32, ("time", "realisation", "lat", "lon")
                )
                fout.description = (
                    "HWMI index (Russo et al. 2014) for "
                    + season
                    + " computed in cross validation"
                )
                fout.history = (
                    "computed from python script by C.Prodhomme & S.Lecestre"
                ...
                expercentfile = fout.createVariable(
                    "nbdaygtpercentpct_" + var,
                    np.float32,
                    ("time", "realisation", "lat", "lon"),
                )
            ...
            try:
                ...
                HWMIaux = HWMI[i : i + 1, 0:1, :, :]  # noqa
                HWMIfile[0:1, 0:1, :, :] = HWMIaux

                # Write the number of days
                exedaux = ndayexedthreshold[i : i + 1, j : j + 1, :, :]  # noqa
                expercentfile[0:1, 0:1, :, :] = exedaux  # noqa
            ...
            try:
                ...
                plotout = tempfile.mktemp(
                    suffix=".png", prefix="heatwaveindex_", dir=workdir
                )
            ...
            try:
                ...
                data = plt.contourf(
                    np.squeeze(lons_reg),
                    np.squeeze(lats_reg),
                    np.squeeze(HWMIaux[0, 0, :, :]),
                    levels=np.linspace(0, 5, 11),
                    transform=ccrs.PlateCarree(),
                    cmap="jet",
                )
                cb = fig.colorbar(data, orientation="horizontal", ax=ax, shrink=0.8)
