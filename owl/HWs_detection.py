...
import shutil
import sys
from owl.function_read import * # from function_read import *
import joblib
...
def calc_HW_MY(mod, mask, lat, lon, args, allowdist=1):
    ...
    for imemb in range(nmemb):
        ...
        for iyear in range(nyear):
            ...
            nHW_i = len(HW)
            dirout = "/cnrm/pastel/USERS/lecestres/NO_SAVE/data/"+ expname +'/' + memb_str + '/Ampli_Fields_'+ parameters_str +'/'
            if not(os.path.isdir(dirout)):
                os.makedirs(dirout)
            fonction_export_allHWs(iyear, nHW_i, dirout, HWampli, fieldHWlst, args)
            print('time for year ', start_year + iyear,' : ', time.time()-start_time_i)
